  ...
  try {
    ...
    if (recentOtp) {
      ...
      if (elapsedSeconds < 60) {
        ...
        const readableTime = formatDistanceToNow(nextEligibleTime, {
          addSuffix: true,
        });

        throw new Error(
          `A verification code was recently sent. You can request a new one ${readableTime}`,
        );
      }
...
export async function verifyOtp(email: string, otp: string) {
  try {
    ...
    if (new Date() > new Date(otpRecord.expiresAt)) {
      ...
      await database.delete(schema.otps).where(eq(schema.otps.email, email));
